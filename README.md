...
- Fail2Ban: aggressive sshd jail, `backend = systemd` (these images ship
  ...
  tail).
- The CI deploy user (`deploy`) has no sudo, is in the `docker` group only,
  and its password field is empty rather than locked — `UsePAM no` makes
  sshd reject pubkey auth outright on a *locked* account regardless of key
  validity, which is not the same failure mode as `PasswordAuthentication
  no` blocking password logins. Both approaches fully block password
  login; only one of them doesn't also break key-based login.
- Dokploy's own Remote Server connections use a separate root keypair,
  generated by Dokploy itself, added to each node's `root` `authorized_keys`
  once during setup — `deploy` has no sudo, and Dokploy needs root or
  passwordless sudo to manage a server.
...
- Dokploy's own control plane was uncapped out of the box and measured at
  roughly 900MB on a 2GB node before `cap-dokploy-resources.sh` bounded
  it — the control plane alone, before any app workload is counted.
